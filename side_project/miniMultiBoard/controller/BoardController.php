...
class BoardController extends Controller { // extends 상속
    ...
    private $boardType = "0"; // 보드 타입 설정

    // getter / setter 하는 이유 : 캡슐화하여 보호
    // boardType getter
    ...
    protected function listGet() {
        ...
        // $b_type = isset($_GET["b_type"]) ? $_GET["b_type"] : "0";   -> 위, 아래 두 가지 방법

        // 보드타입 프로퍼티 셋
        $this->setBoardType($requestData["b_type"]);


        // 유효성 체크는 시간남으면 도전 TODO
    ...
    public function addPost() {
        // 이미지 파일 처리
        $path = ""; // 이미지 파일 없이 출력하기 위해 빈 문자열 설정
    ...
    public function detailGet() {
        ...
        // JSON 변환
        $resonse = json_encode($resultBoard);
